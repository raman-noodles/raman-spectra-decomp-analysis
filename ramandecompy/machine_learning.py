...
def generate_spectra_dataset(hdf5_filename, target_compound, spectra_count):
    """
    A function that generates a set of raman spectra which consist of a sum of normalized calibration
    spectra. Each spectra contains all of the calibration spectra found in the hdf5 file specified
    which are first interpolated and normalized, then scaled from 0.0 to 1.0 before being added together.
    Every odd spectra will contain the target_compound at a 0.1 to 1.0 scaling while each even spectra
    will not contain the target_compound.
    
    Args:
        hdf5_filename (str): The name and location of the relevant hdf5 calibration datafile.
        target_compound (str): The compound key in the hdf5 calibration datafile that will alternate
                               as present and not present throughout the dataset.
        spectra_count (int): An integer equal to the number of spectra desired for the dataset.
        
    Returns:
        x_data (list): A list of the wavenumber values for the generated spectra. All elements of the
                       list should be identical and are primarily used for identifying the wavenumber
                       range of the dataset.
        y_data (list): A list of the counts values for the generated spectra.
        label (list): A list of the labels for each spectra indicating the presence or lack of the
                      target_compound; (0 = no target, 1 = yes target).
    """
    hdf5 = h5py.File(hdf5_filename, 'r')
...
def interpolated_spectra(hdf5_interpfilename, hdf5_calfilename, spectra_count):
    ...
    for _, target_compound in enumerate(compound_list):
        x_data, y_data, labels = machine_learning.generate_spectra_dataset(hdf5_calfilename,
                                                                           target_compound, spectra_count)
        y_data_list.append(y_data)
        x_data_list.append(x_data)
        for i, label in enumerate(labels):
            interpdf = machine_learning.combine_experiment(hdf5_interpfilename, 'interp_'+target_compound,
